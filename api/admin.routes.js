...
r.get("/items", async (req, res) => {
  try {
    const { search = "", status, category, page = 1, limit = 20, sort = "createdAt:desc", includeDeleted = "0" } = req.query;
    const filter = {};
    ...
    s[sp[0] || "createdAt"] = (sp[1] || "desc") === "asc" ? 1 : -1;
  ...
  } catch (e) { res.status(500).json({ error: "Gagal ambil items", detail: e.message }); }
});
r.post("/items", async (req, res) => {
  ...
  } catch (e) { res.status(400).json({ error: "Gagal menambah", detail: e.message }); }
});
r.patch("/items/:id", async (req, res) => {
  try {
    ...
    await ChangeLog.create({ refCollection: "items", refId: doc._id, action: "update", user: req.actor, diff: { before, after: doc } });
    res.json(doc);
  } catch (e) { res.status(400).json({ error: "Gagal update", detail: e.message }); }
});
r.delete("/items/:id", async (req, res) => {
  try {
    ...
    res.json({ ok: true });
  } catch (e) { res.status(400).json({ error: "Gagal hapus", detail: e.message }); }
});
r.post("/items/:id/restore", async (req, res) => {
  try {
    ...
    res.json(restored);
  } catch (e) { res.status(400).json({ error: "Gagal restore", detail: e.message }); }
});
r.post("/items/bulk", async (req, res) => {
  try {
    ...
    await ChangeLog.create({ refCollection: "items", refId: null, action: "update", user: req.actor, diff: { bulk: true, action, ids, payload } });
    res.json({ ok: true, result });
  } catch (e) { res.status(400).json({ error: "Bulk gagal", detail: e.message }); }
});
export default r;
