...
const allowedOrigins = new Set(
  [CORS_ORIGIN, FRONTEND_URL, "http://localhost:5173"].filter(Boolean)
);
...
app.get("/test", (_req, res) => res.json({ message: "API is running (Vercel)" }));
...
app.post("/items", adminOnly, async (req, res) => {
  try { const item = await Item.create(req.body); res.status(201).json({ message: "Produk berhasil ditambahkan", item }); }
  catch (err) { res.status(500).json({ error: "Gagal menambahkan produk", detail: err.message }); }
});
app.put("/items/:id", adminOnly, async (req, res) => {
  try {
    ...
    res.json(updated);
  } catch (err) { res.status(500).json({ error: "Gagal update", detail: err.message }); }
});
...
app.post("/checkout", authMiddleware, async (req, res) => {
  try {
    ...
    const newOrder = await Order.create({
      user: userId, username, email, address, whatsapp,
